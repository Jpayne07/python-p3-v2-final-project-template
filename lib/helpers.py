...
def view_applicants(menu, current_job=None):
    ...
    if choice == 1:
        i=1
        print("---\nAPPLICANTS")
        if len(current_job.applicants()) > 0:
            for applicant in current_job.applicants():
                if applicant:
                    print(f"\n{i}. {applicant.name}")
                    i+=1
        else:
...
def create_new_application(menu): #create new application1
    name = input("Enter name: ")
    try:
        job_id = int(input("\nEnter job ID: "))#take in a job object instead of job id
...
def view_app(menu, selected_app): #fetch application by id
    print(f"---\nNow viewing {selected_app.name}'s application for Job ID: {selected_app.job_id}\n---")
